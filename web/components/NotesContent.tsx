...
import { Sparkles, Target, Clock, Expand } from "lucide-react";
...
export default function NotesContent({
  noteId, title, summary, chapters, currentTime, onSeek,
}: Props) {
  const { lang } = useLang();
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[var(--fg-secondary)]">
          ...
          <span>·</span>
          <span>{chapters.length} 章 / {summary.length} 段</span>
        </div>
        {keywords.length > 0 && (
      ...
      {/* 知识点速览（卡片网格） */}
      <section>
        <h2 className="text-lg font-semibold mb-3 text-[var(--fg)]">💡 知识点速览</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {summary.map((c, i) => {
            const marks = chunkMarks(c);
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(() => {
                    // 段内容预览：优先 summary_{lang}，缺则 text_{lang} 截前 120 字
                  ...
                  {(() => {
                    // 关键词条 — 优先 keywords_{lang}，缺则原 keywords
                    const langKws =
      ...
      <section className="flex flex-col gap-5">
        <h2 className="text-lg font-semibold text-[var(--fg)]">📑 章节</h2>
        {chapters.map((ch, ci) => {
          ...
          return (
            ...
            >
              ...
              {hasChildren ? (
                /* 有子章节：渲染嵌套小卡，不再画 chunk chips（避免重复，子卡自己画） */
      ...
      {glossary.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold mb-3 text-[var(--fg)]">📚 术语表</h2>
