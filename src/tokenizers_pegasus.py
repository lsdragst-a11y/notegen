...
# 原始 tokenizers_pegasus.py 从 fengshen.examples.pegasus.data_utils 导入下面 4 个
# 辅助函数，那个包路径在我们的项目里不存在，所以把它们 inline 到这里。
# 实现完全来自 IDEA-CCNL/Randeng-Pegasus-238M-Summary-Chinese 仓库的 data_utils.py。
...
class PegasusTokenizer(PreTrainedTokenizer):
    ...
    def __init__(self,
        ...
        # transformers 4.x 在 super().__init__() 内部会通过 _add_tokens → get_vocab()
        # 访问 self.vocab，所以必须在 super 之前完成 vocab 装载与重命名。
        self.vocab = load_vocab(vocab_file)
        self.vocab[eos_token] = self.vocab.pop("[unused1]")
        self.vocab[pad_token] = self.vocab.pop("[PAD]")
        self.vocab[unk_token] = self.vocab.pop("[UNK]")
        if mask_token_sent is not None:
            self.vocab[mask_token] = self.vocab.pop("[unused3]")
            self.vocab[mask_token_sent] = self.vocab.pop("[unused2]")
